...
- [ ] Schema parses valid JSON into ParameterSchema
- [ ] Boolean flag renders as `--flag` (when true) or omitted (when false)
- [ ] String value renders as `--flag value`
- [ ] Array renders as multiple `--flag value` pairs
- [ ] Map renders as `--prefix key=value` for each entry
- [ ] Unknown parameter type returns error
- [ ] Missing required parameter returns error
- [ ] Multi-value flag with no value returns empty list
...
- [ ] AC1: `cargo test --manifest-path src-tauri/Cargo.toml` passes with new renderer tests
- [ ] AC2: All providers (dotnet, cargo, go, java) have `parameters.json` manifests
- [ ] AC3: Rendering produces correct CLI syntax for all parameter types
- [ ] AC4: Invalid parameters produce clear error messages

...
const BANNED = [
  [
    "bare-rounded",
    /(?<![-\w])rounded(?![-\w])/,
    "裸 rounded=4px，用 rounded-sm",
  ],
  [
    "rounded-lg-or-xl",
    /rounded-(lg|xl|2xl|3xl)\b/,
    "16px+ 仅限全屏面，控件用 rounded-sm，模态 rounded-md",
  ],
  [
    "stock-text-size",
    /text-(xs|sm|base|lg|xl|2xl)\b/,
    "禁原生字号，用 text-label-*/copy-*/heading-*/button-*",
  ],
  ...
  [
    "opacity-disabled",
    /disabled:opacity-\d+/,
    "禁用态用 gray-100 底 + gray-700 字",
  ],
  [
    "stock-palette",
    /\b(bg|text|border)-(slate|zinc|stone|neutral|sky|indigo|violet|rose|orange|lime|emerald|cyan|fuchsia)-\d+/,
    "禁原生调色板",
  ],
...
const ALLOWLIST = [
  ...
  {
    file: "src/components/layout/PublishContentSection.tsx",
    rule: "decorative-anim",
    reason: "懒加载占位加载中指示",
  },
  {
    file: "src/components/ui/switch.tsx",
    rule: "opacity-disabled",
    reason: "Radix Switch 整体禁用惯例",
  },
  {
    file: "src/components/publish/PublishRunCard.tsx",
    rule: "opacity-disabled",
    reason: "取消钮 disabled:opacity-100 保持不透明非降调",
  },
];

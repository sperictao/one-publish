...
fn composition_contains_sensitive_settings(composition: &crate::store::PublishComposition) -> bool {
...
mod tests {
    ...
    fn validate_rejects_project_binding_not_owned_by_provider() {
        for (project_binding, provider_version) in [
            ...
            let error = validate_import(&config).expect_err(

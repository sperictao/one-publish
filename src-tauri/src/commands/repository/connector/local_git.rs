...
use super::super::{repository_error, RepositoryBranchScanResult};
...
fn section_matches(line: &str, section: &str, subsection: Option<&str>) -> bool {
    ...
    if inner.len() < section.len() || !inner[..section.len()].eq_ignore_ascii_case(section) {
        return false;
    }

    let rest = inner[section.len()..].trim();

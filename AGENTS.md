...
- **Research**（未验证调研）：`docs/agent/`，文件名以 `R-<n>-` 开头，带 TOML frontmatter。
- **Evolve**（进行中的变更）：新工作走 GitHub Issues/PRs（见上文 Issue tracker），不再新增 `plans/` 文件。`plans/` 是已冻结的历史执行台账（E-1 至 E-35），仅供追溯。
